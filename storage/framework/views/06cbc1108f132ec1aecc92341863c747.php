<?php $__env->startSection('title', isset($article) ? '기사 수정' : '기사 작성'); ?>
...
<div class="af-header">
    <h1 class="wp-page-title" style="margin-bottom:0;">
        <?php echo e(isset($article) ? '기사 수정' : '새 기사 작성'); ?>

    </h1>
    <a href="<?php echo e(route('admin.articles')); ?>" class="wp-btn wp-btn-secondary">← 목록으로</a>
</div>
    ...
    <div class="af-grid">
        ...
        <div class="af-main-col">
                ...
                <div class="wp-widget-body">
                    ...
                    <div class="wp-form-group" style="margin-bottom:0;">
                        ...
                        <input type="text" name="slug" id="slug" class="wp-form-input"
                               ...
                               value="<?php echo e(old('slug', $article->slug ?? '')); ?>" maxlength="300">
            ...
            <div class="wp-widget" style="margin-bottom:16px;">
                ...
                <div class="wp-widget-body">
                    <textarea name="content" id="se2_content" style="width:100%;height:520px;display:none;"><?php echo e(old('content', $article->content ?? '')); ?></textarea>
                </div>
            </div>
            ...
            <div class="wp-widget">
                ...
                <div class="wp-widget-body">
                    <textarea name="excerpt" class="wp-form-input wp-form-textarea"
                              ...
                              placeholder="목록에 표시될 요약 문장 (비워두면 본문에서 자동 생성 가능)"><?php echo e(old('excerpt', $article->excerpt ?? '')); ?></textarea>
        ...
        <div class="af-side-col">
                ...
                <div class="wp-widget-body af-panel-body open">
                    <div class="wp-form-group">
                        ...
                        <select name="status" id="status" class="wp-form-input wp-form-select">
                            <option value="draft"     <?php echo e(old('status', $article->status ?? 'draft') === 'draft'     ? 'selected' : ''); ?>>초안</option>
                            <option value="pending"   <?php echo e(old('status', $article->status ?? '') === 'pending'   ? 'selected' : ''); ?>>승인 대기</option>
                            <option value="published" <?php echo e(old('status', $article->status ?? '') === 'published' ? 'selected' : ''); ?>>게시됨</option>
                        </select>
                    </div>
                    <div class="wp-form-group" id="published-at-group"
                         style="<?php echo e(in_array(old('status', $article->status ?? 'draft'), ['published']) ? '' : 'display:none;'); ?>">
                        <label class="wp-form-label">게시 일시</label>
                        <input type="datetime-local" name="published_at" class="wp-form-input"
                               value="<?php echo e(old('published_at', isset($article->published_at) ? $article->published_at->format('Y-m-d\TH:i') : '')); ?>">
                    ...
                    <div class="af-desktop-save" style="display:flex;gap:8px;margin-top:16px;">
                        <button type="submit" class="wp-btn wp-btn-primary" style="flex:1;">
                            <?php echo e(isset($article) ? '저장' : '등록'); ?>

                        </button>
                        <a href="<?php echo e(route('admin.articles')); ?>" class="wp-btn wp-btn-secondary">취소</a>
                    </div>
                </div>
            </div>
                ...
                <div class="wp-widget-body af-panel-body open">
                    <?php if($categories->isEmpty()): ?>
                        <p style="font-size:13px;color:#8c8f94;">
                            등록된 카테고리가 없습니다.
                            <a href="<?php echo e(route('admin.category.create')); ?>" style="color:#2271b1;">추가하기</a>
                        </p>
                    <?php else: ?>
            ...
            <div class="wp-widget" style="margin-bottom:16px;">
                ...
                <div class="wp-widget-body af-panel-body open">
                    <div id="thumbnail-preview" style="margin-bottom:10px;display:<?php echo e(old('thumbnail', $article->thumbnail ?? '') ? 'block' : 'none'); ?>;">
                        <img id="thumbnail-img"
                             src="<?php echo e(old('thumbnail', $article->thumbnail ?? '')); ?>"
                    ...
                    <input type="hidden" name="thumbnail" id="thumbnail-url"
                           value="<?php echo e(old('thumbnail', $article->thumbnail ?? '')); ?>">
                    ...
                    <div class="wp-widget-body" style="border-top:1px solid #f0f0f1;">
                        <div class="wp-form-group">
                            ...
                            <input type="text" name="focus_keyword" id="focus-keyword" class="wp-form-input"
                                   ...
                                   value="<?php echo e(old('focus_keyword', $article->focus_keyword ?? '')); ?>">
                        </div>
                        <div class="wp-form-group">
                            ...
                            <div id="serp-preview" style="border:1px solid #e0e0e0;border-radius:8px;padding:12px 14px;background:#fff;font-family:arial,sans-serif;">
                                <div style="font-size:11px;color:#202124;margin-bottom:3px;">
                                    <span style="color:#0f9d58;"><?php echo e(parse_url(url('/'), PHP_URL_HOST)); ?></span> › news › ...
                                </div>
                        ...
                        <div class="wp-form-group">
                            ...
                            <input type="text" name="meta_title" id="seo-title" class="wp-form-input"
                                   ...
                                   value="<?php echo e(old('meta_title', $article->meta_title ?? '')); ?>">
                        ...
                        <div class="wp-form-group">
                            ...
                            <textarea name="meta_description" id="seo-desc" class="wp-form-input wp-form-textarea"
                                      ...
                                      placeholder="비워두면 발췌문 또는 본문 앞부분 사용"><?php echo e(old('meta_description', $article->meta_description ?? '')); ?></textarea>
                            <div style="display:flex;justify-content:space-between;margin-top:3px;">
                        ...
                        <div class="wp-form-group">
                            ...
                            <input type="url" name="og_image" id="og-image-url" class="wp-form-input"
                                   ...
                                   value="<?php echo e(old('og_image', $article->og_image ?? '')); ?>">
...
<div class="af-sticky-bar">
    ...
    <a href="<?php echo e(route('admin.articles')); ?>" class="wp-btn wp-btn-secondary" style="height:40px;line-height:28px;">취소</a>
</div>

<?php echo $__env->make('partials.smarteditor', ['editorId' => 'se2_content', 'editorHeight' => 520], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<?php echo $__env->make('admin.partials.media-picker', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<?php echo $__env->make('admin.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/laraboard/www/resources/views/admin/article-form.blade.php ENDPATH**/ ?>