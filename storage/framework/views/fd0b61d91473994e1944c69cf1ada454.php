<?php $__env->startSection('title', '기사 관리'); ?>
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:8px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <?php if(!$isTrash): ?>
        <a href="<?php echo e(route('admin.article.export', request()->only('status','category_id'))); ?>"
        ...
        <a href="<?php echo e(route('admin.article.create')); ?>" class="wp-btn wp-btn-primary">+ 새 기사</a>
        <?php else: ?>
        
        <?php if($counts['trash'] > 0): ?>
        <form method="POST" action="<?php echo e(route('admin.article.empty-trash')); ?>" style="display:inline;"
              onsubmit="return confirm('휴지통의 기사 <?php echo e($counts['trash']); ?>개를 모두 영구 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.')">
            <?php echo csrf_field(); ?>
            <button class="wp-btn wp-btn-danger">🗑 휴지통 비우기 (<?php echo e($counts['trash']); ?>개)</button>
        </form>
        <?php endif; ?>
        <?php endif; ?>
    </div>
...
<div id="import-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:6px;padding:28px 32px;width:100%;max-width:480px;box-shadow:0 8px 30px rgba(0,0,0,.2);">
        ...
        <form method="POST" action="<?php echo e(route('admin.article.import')); ?>" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
...
<div style="display:flex;gap:0;margin-bottom:16px;border-bottom:1px solid #c3c4c7;flex-wrap:wrap;">
    ...
    <?php $__currentLoopData = ['' => '전체', 'published' => '게시됨', 'pending' => '승인 대기', 'draft' => '초안']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <a href="<?php echo e(route('admin.articles', array_merge(request()->except('status','page'), $val ? ['status'=>$val] : []))); ?>"
       style="padding:8px 14px;font-size:13px;text-decoration:none;border-bottom:2px solid transparent;margin-bottom:-1px;
              <?php echo e((!$isTrash && $curStatus === $val) ? 'border-bottom-color:#2271b1;color:#2271b1;font-weight:700;' : 'color:#646970;'); ?>">
        <?php echo e($label); ?>

        <span style="font-size:11px;background:#f0f0f1;padding:1px 6px;border-radius:10px;margin-left:4px;">
            <?php echo e($counts[$val === '' ? 'all' : $val]); ?>

        </span>
    </a>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    
    <a href="<?php echo e(route('admin.articles', ['status' => 'trash'])); ?>"
       style="padding:8px 14px;font-size:13px;text-decoration:none;border-bottom:2px solid transparent;margin-bottom:-1px;
              <?php echo e($isTrash ? 'border-bottom-color:#d63638;color:#d63638;font-weight:700;' : 'color:#646970;'); ?>">
        🗑 휴지통
        <?php if($counts['trash'] > 0): ?>
        <span style="font-size:11px;background:#fce8e8;color:#d63638;padding:1px 6px;border-radius:10px;margin-left:4px;">
            <?php echo e($counts['trash']); ?>
...
<div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
    <?php if(!$isTrash): ?>
    <form method="GET" action="<?php echo e(route('admin.articles')); ?>" style="display:flex;gap:6px;flex-wrap:wrap;">
        ...
        <input type="text" name="search" class="wp-form-input" style="max-width:240px;"
               placeholder="제목 또는 내용 검색..." value="<?php echo e(request('search')); ?>">
        <button type="submit" class="wp-btn wp-btn-secondary">검색</button>
        <?php if(request()->hasAny(['search','category_id'])): ?>
            <a href="<?php echo e(route('admin.articles', request()->only('status'))); ?>" class="wp-btn wp-btn-secondary">초기화</a>
        <?php endif; ?>
    </form>
    <?php endif; ?>

    
    <form id="bulk-form" method="POST" action="<?php echo e(route('admin.article.bulk')); ?>" style="display:flex;gap:6px;align-items:center;margin-left:auto;">
        <?php echo csrf_field(); ?>
    ...
    <div class="wp-widget-body" style="padding:0;">
        ...
        <table class="wp-list-table">
            ...
            <tbody>
                ...
                <tr id="article-row-<?php echo e($article->id); ?>" style="<?php echo e($isTrash ? 'opacity:.8;' : ''); ?>">
                    <td style="padding:10px 12px;">
                        <input type="checkbox" class="row-check" name="ids[]"
                               form="bulk-form" value="<?php echo e($article->id); ?>"
                               onchange="updateSelectedCount()">
                    </td>
                    <td>
                        <strong style="font-size:13px;"><?php echo e($article->title); ?></strong>
                        <?php if($article->excerpt && !$isTrash): ?>
                    ...
                    <td style="color:#646970;font-size:12px;"><?php echo e($article->category?->name ?? '—'); ?></td>
                    <td style="font-size:12px;"><?php echo e($article->user->name); ?></td>

                    <?php if($isTrash): ?>
                    <td style="font-size:12px;color:#d63638;"><?php echo e($article->deleted_at->format('Y-m-d H:i')); ?></td>
                    <?php else: ?>
                    <td><span class="wp-badge <?php echo e($article->statusClass()); ?>"><?php echo e($article->statusLabel()); ?></span></td>
                    <td style="font-size:12px;color:#646970;">
                        <?php echo e($article->created_at->format('Y-m-d')); ?>

                        <?php if($article->published_at): ?>
                            <div style="color:#00a32a;">공개 <?php echo e($article->published_at->format('m-d')); ?></div>
                        <?php endif; ?>
                    </td>
                    <?php endif; ?>
                    ...
                    <td>
                        <?php if($isTrash): ?>
                        <form method="POST" action="<?php echo e(route('admin.article.restore', $article->id)); ?>" style="display:inline;">
                            <?php echo csrf_field(); ?>
                            <button class="wp-btn wp-btn-secondary wp-btn-sm">↩ 복원</button>
                        </form>
                        <form method="POST" action="<?php echo e(route('admin.article.force-delete', $article->id)); ?>" style="display:inline;"
                              onsubmit="return confirm('「<?php echo e(addslashes($article->title)); ?>」을(를) 영구 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.')">
                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                            <button class="wp-btn wp-btn-danger wp-btn-sm">영구 삭제</button>
                        </form>
                        <?php else: ?>
                        <?php if($article->status !== 'published'): ?>
                        <form method="POST" action="<?php echo e(route('admin.article.status', $article->id)); ?>" style="display:inline;">
                            <?php echo csrf_field(); ?>
                        ...
                        <?php endif; ?>
                        <a href="<?php echo e(route('admin.article.edit', $article->id)); ?>" class="wp-btn wp-btn-secondary wp-btn-sm">수정</a>
                        <form method="POST" action="<?php echo e(route('admin.article.delete', $article->id)); ?>" style="display:inline;"
                              onsubmit="return confirm('「<?php echo e(addslashes($article->title)); ?>」을(를) 휴지통으로 이동하시겠습니까?')">
                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                ...
                <tr>
                    <td colspan="7" style="text-align:center;padding:40px;color:#8c8f94;">
                        <?php if($isTrash): ?> 휴지통이 비어있습니다.
                        <?php elseif(request()->hasAny(['search','category_id','status'])): ?> 검색 결과가 없습니다.
                        <?php else: ?> 등록된 기사가 없습니다.
                            <a href="<?php echo e(route('admin.article.create')); ?>" style="color:#2271b1;">첫 기사를 작성해보세요.</a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endif; ?>
...
<?php echo $__env->make('admin.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/laraboard/www/resources/views/admin/articles.blade.php ENDPATH**/ ?>