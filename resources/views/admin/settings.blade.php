...
<form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="wp-widget-body">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 24px;">
                <div class="wp-form-group">
                    ...
                    <input type="text" name="logo_text" value="{{ old('logo_text', $settings['logo_text']) }}" class="wp-form-input" placeholder="사이트 이름을 입력하면 기본값 사용">
                    <p class="wp-form-help">비워두면 사이트 이름으로 표시됩니다.</p>
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">태그라인 (로고 아래 문구)</label>
                    <input type="text" name="logo_tagline" value="{{ old('logo_tagline', $settings['logo_tagline']) }}" class="wp-form-input" placeholder="예: 대한민국 대표 커뮤니티">
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 24px;">
                ...
                <div class="wp-form-group">
                    ...
                    <div style="margin-bottom:8px;padding:8px;border:1px solid #c3c4c7;border-radius:3px;background:#f6f7f7;display:inline-flex;align-items:center;gap:10px;">
                        <img src="{{ $settings['logo_image'] }}" alt="현재 로고" style="max-height:40px;max-width:160px;object-fit:contain;">
                    ...
                    @error('logo_image_file') <p style="color:#d63638;font-size:12px;margin-top:4px;">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="wp-form-group">
                    ...
                    @if($settings['favicon'])
                    <div style="margin-bottom:8px;padding:8px;border:1px solid #c3c4c7;border-radius:3px;background:#f6f7f7;display:inline-flex;align-items:center;gap:10px;">
                        <img src="{{ $settings['favicon'] }}" alt="현재 파비콘" style="width:32px;height:32px;object-fit:contain;">
                        <label style="display:flex;align-items:center;gap:5px;font-size:12px;color:#d63638;cursor:pointer;">
                    ...
                    @error('favicon_file') <p style="color:#d63638;font-size:12px;margin-top:4px;">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="wp-widget" style="margin-bottom:16px;">
        ...
        <div class="wp-widget-body">
            ...
            <div style="margin-bottom:16px;padding:12px 16px;background:#1d2327;border-radius:4px;display:inline-flex;align-items:center;gap:10px;">
                @if($settings['admin_logo_image'])
                    <img src="{{ $settings['admin_logo_image'] }}" alt="관리자 로고" style="max-height:32px;max-width:140px;object-fit:contain;">
                @else
                    ...
                    <span style="color:#fff;font-weight:900;font-size:15px;">{{ $settings['admin_logo_text'] ?: 'Laraboard' }}</span>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 24px;">
                <div class="wp-form-group">
                    ...
                    @if($settings['admin_logo_image'])
                    <div style="margin-bottom:8px;padding:8px;border:1px solid #c3c4c7;border-radius:3px;background:#f6f7f7;display:inline-flex;align-items:center;gap:10px;">
                        <img src="{{ $settings['admin_logo_image'] }}" alt="관리자 로고" style="max-height:36px;max-width:140px;object-fit:contain;">
                        <label style="display:flex;align-items:center;gap:5px;font-size:12px;color:#d63638;cursor:pointer;">
                    ...
                    @error('admin_logo_image_file') <p style="color:#d63638;font-size:12px;margin-top:4px;">{{ $message }}</p> @enderror
                </div>

                <div class="wp-form-group">
                    <label class="wp-form-label">관리자 로고 텍스트</label>
                    <input type="text" name="admin_logo_text" value="{{ old('admin_logo_text', $settings['admin_logo_text']) }}" class="wp-form-input" placeholder="Laraboard">
    ...
    <div class="wp-widget" style="margin-bottom:16px;">
        ...
        <div class="wp-widget-body">
            <div class="wp-form-group">
                ...
                <input type="text" name="site_name" value="{{ old('site_name', $settings['site_name']) }}" required class="wp-form-input">
            </div>
            <div class="wp-form-group">
                <label class="wp-form-label">사이트 설명</label>
                <textarea name="site_description" rows="3" class="wp-form-input wp-form-textarea">{{ old('site_description', $settings['site_description']) }}</textarea>
            </div>
            <div class="wp-form-group">
                <label class="wp-form-label">사이트 키워드</label>
                <input type="text" name="site_keywords" value="{{ old('site_keywords', $settings['site_keywords']) }}" class="wp-form-input">
    ...
    <div class="wp-widget" style="margin-bottom:16px;border-left:3px solid #2271b1;">
        <div class="wp-widget-body" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
            ...
            <a href="{{ route('admin.seo') }}" class="wp-btn wp-btn-primary" style="white-space:nowrap;">SEO 설정으로 이동 →</a>
        </div>
    </div>
        ...
        <div class="wp-widget-body">
            <div class="wp-form-group">
                ...
                <select name="layout_skin" class="wp-form-input wp-form-select">
                    @foreach($layoutSkins as $key => $name)
                        <option value="{{ $key }}" {{ old('layout_skin', $settings['layout_skin']) == $key ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            ...
            <div class="wp-form-group">
                ...
                <select name="board_skin" class="wp-form-input wp-form-select">
                    @foreach($boardSkins as $key => $name)
                        <option value="{{ $key }}" {{ old('board_skin', $settings['board_skin']) == $key ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="wp-form-group">
                ...
                <select name="member_skin" class="wp-form-input wp-form-select">
                    @foreach($memberSkins as $key => $name)
                        <option value="{{ $key }}" {{ old('member_skin', $settings['member_skin']) == $key ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="wp-widget" style="margin-bottom:16px;">
        ...
        <div class="wp-widget-body">
            <div class="wp-form-group">
                ...
                <textarea name="custom_head_script" rows="5" class="wp-form-input wp-form-textarea" style="font-family:monospace;font-size:12px;">{{ old('custom_head_script', $settings['custom_head_script']) }}</textarea>
                <p class="wp-form-help">Google Analytics, 폰트, CSS 등</p>
            </div>
            <div class="wp-form-group">
                <label class="wp-form-label">BODY 영역 스크립트</label>
                <textarea name="custom_body_script" rows="5" class="wp-form-input wp-form-textarea" style="font-family:monospace;font-size:12px;">{{ old('custom_body_script', $settings['custom_body_script']) }}</textarea>
            ...
            <div class="wp-form-group">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:600;">
                    <input type="checkbox" name="comments_enabled" value="1"
                           {{ old('comments_enabled', $settings['comments_enabled']) == '1' ? 'checked' : '' }}>
                    기사 댓글 기능 활성화
                </label>
            ...
            <div class="wp-form-group">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:600;">
                    <input type="checkbox" name="comments_require_login" value="1"
                           {{ old('comments_require_login', $settings['comments_require_login']) == '1' ? 'checked' : '' }}>
                    댓글 작성 시 로그인 필요
                </label>
            ...
            <div class="wp-form-group">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:600;">
                    <input type="checkbox" name="comments_moderation" value="1"
                           {{ old('comments_moderation', $settings['comments_moderation']) == '1' ? 'checked' : '' }}>
                    댓글 사전 검토 (승인 후 표시)
                </label>
        ...
        <div class="wp-widget-body">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 24px;">
                <div class="wp-form-group">
                    ...
                    <input type="text" name="press_masthead" value="{{ old('press_masthead', $settings['press_masthead']) }}" class="wp-form-input" placeholder="예: 라라보드뉴스">
                    <p class="wp-form-help">언론사 공식 제호</p>
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">등록번호</label>
                    <input type="text" name="press_registration_number" value="{{ old('press_registration_number', $settings['press_registration_number']) }}" class="wp-form-input" placeholder="예: 서울 아 01234">
                    <p class="wp-form-help">인터넷신문 등록번호</p>
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">발행인</label>
                    <input type="text" name="press_publisher" value="{{ old('press_publisher', $settings['press_publisher']) }}" class="wp-form-input" placeholder="홍길동">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">편집인</label>
                    <input type="text" name="press_editor" value="{{ old('press_editor', $settings['press_editor']) }}" class="wp-form-input" placeholder="홍길동">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">대표번호</label>
                    <input type="text" name="press_phone" value="{{ old('press_phone', $settings['press_phone']) }}" class="wp-form-input" placeholder="02-1234-5678">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">팩스번호</label>
                    <input type="text" name="press_fax" value="{{ old('press_fax', $settings['press_fax']) }}" class="wp-form-input" placeholder="02-1234-5679">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">이메일</label>
                    <input type="email" name="press_email" value="{{ old('press_email', $settings['press_email']) }}" class="wp-form-input" placeholder="news@example.com">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">우편번호</label>
                    <input type="text" name="press_postal_code" value="{{ old('press_postal_code', $settings['press_postal_code']) }}" class="wp-form-input" placeholder="12345">
                </div>
            </div>
            <div class="wp-form-group">
                <label class="wp-form-label">회사주소</label>
                <input type="text" name="press_address" value="{{ old('press_address', $settings['press_address']) }}" class="wp-form-input" placeholder="서울특별시 강남구 테헤란로 123, 456호">
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0 24px;">
                <div class="wp-form-group">
                    <label class="wp-form-label">청소년보호책임자</label>
                    <input type="text" name="press_youth_manager" value="{{ old('press_youth_manager', $settings['press_youth_manager']) }}" class="wp-form-input" placeholder="홍길동">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">개인정보 보호책임자</label>
                    <input type="text" name="press_privacy_manager" value="{{ old('press_privacy_manager', $settings['press_privacy_manager']) }}" class="wp-form-input" placeholder="홍길동">
                </div>
                <div class="wp-form-group">
                    <label class="wp-form-label">고충처리인 <span style="color:#8c8f94;font-weight:400;">(선택)</span></label>
                    <input type="text" name="press_grievance_manager" value="{{ old('press_grievance_manager', $settings['press_grievance_manager']) }}" class="wp-form-input" placeholder="홍길동">
                </div>
            </div>
