            ...
            <ul id="banner-sortable" style="list-style:none;margin:0;padding:0;">
                ...
                <li data-id="{{ $banner->id }}"
                    ...
                    <div style="padding:10px 14px 0;display:flex;align-items:center;gap:8px;">
                        ...
                        {{-- 배너 미리보기 --}}
                        <div style="flex:1;border-radius:4px;padding:6px 12px;background:{{ $banner->bg_color }};color:{{ $banner->text_color }};font-size:{{ $banner->font_size }}px;font-weight:{{ $banner->font_weight }};overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">
                            @if($banner->link_url)
                                <a href="{{ $banner->link_url }}" style="color:{{ $banner->text_color }};text-decoration:none;" onclick="return false;">{{ $banner->text }}</a>
                            @else
                                {{ $banner->text }}
                            @endif
                        </div>
                    ...
                    <div style="padding:6px 14px 10px 36px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        ...
                        <span style="font-size:11px;color:#8c8f94;">
                            {{ $banner->start_at ? $banner->start_at->format('Y-m-d') : '∞' }}
                            ~
                            {{ $banner->end_at   ? $banner->end_at->format('Y-m-d')   : '∞' }}
                        </span>
                        @endif
                        <span style="font-size:11px;color:#8c8f94;">
                            닫기 후 {{ $banner->reshow_hours == 0 ? '다시 안 보임' : $banner->reshow_hours.'시간 후 재표시' }}
                        </span>
                        <div style="margin-left:auto;display:flex;gap:4px;">
                            <button type="button"
                                    onclick="openEditModal({{ $banner->id }}, {{ json_encode($banner->text) }}, {{ json_encode($banner->link_url ?? '') }}, '{{ $banner->text_color }}', '{{ $banner->bg_color }}', {{ $banner->font_size }}, '{{ $banner->font_weight }}', '{{ $banner->start_at ? $banner->start_at->format('Y-m-d\TH:i') : '' }}', '{{ $banner->end_at ? $banner->end_at->format('Y-m-d\TH:i') : '' }}', {{ $banner->reshow_hours }}, {{ $banner->is_active ? 'true' : 'false' }})"
                                    class="wp-btn wp-btn-secondary wp-btn-sm">수정</button>
                            <form method="POST" action="{{ route('admin.top-banner.delete', $banner->id) }}" onsubmit="return confirm('삭제하시겠습니까?');" style="display:inline;">
                                @csrf @method('DELETE')
    ...
    <div>
        <div class="wp-widget">
            ...
            <div class="wp-widget-body">
                <form method="POST" action="{{ route('admin.top-banner.store') }}" id="add-form">
                    @csrf
