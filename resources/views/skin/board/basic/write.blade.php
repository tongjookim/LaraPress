...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">글쓰기</h1>
        <p class="text-gray-500 mt-2">{{ $board->board_name }}</p>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <form method="POST" action="{{ route('bbs.store', $board->board_id) }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea name="content" id="se2_content" style="width:100%;height:450px;display:none;">{{ old('content') }}</textarea>
                @error('content')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
