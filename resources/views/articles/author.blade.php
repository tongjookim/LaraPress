...
<div class="j-hero">
    <div class="j-hero-inner">
        @if($author->profile_image)
            <img src="{{ $author->profile_image }}" alt="{{ $author->name }}" class="j-hero-avatar">
        @else
            <div class="j-hero-avatar">{{ mb_substr($author->name, 0, 1) }}</div>
        @endif
        <div class="j-hero-info">
            <div class="j-hero-role">{{ $author->roleLabel() }}</div>
            <div class="j-hero-name">{{ $author->name }}</div>
            <div class="j-hero-username">{{ '@'.$author->username }}</div>
            <div class="j-hero-stats">
                <div class="j-stat">
                    <div class="j-stat-num">{{ number_format($totalArticles) }}</div>
                    <div class="j-stat-label">기사</div>
                </div>
                <div class="j-stat">
                    <div class="j-stat-num">{{ number_format($totalViews) }}</div>
...
<div class="j-layout">
    ...
    <aside class="j-sidebar">
        ...
        <div class="j-card">
            ...
            <div class="j-card-body" style="padding:8px;">
                <div class="j-social-list">
                    @foreach($socialDefs as $field => $def)
                    @if(!empty($author->$field))
                    @php $href = $def['prefix'] ? $def['prefix'].$author->$field : $author->$field; @endphp
                    <a href="{{ $href }}" target="{{ str_starts_with($href,'mailto') ? '_self' : '_blank' }}" rel="noopener noreferrer" class="j-social-link">
                        <span class="j-social-icon" style="background:{{ $def['bg'] }};color:{{ $def['color'] }};">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="{{ in_array($field,['social_facebook','social_x','social_pixabay','social_wikipedia']) ? 'currentColor' : 'none' }}" stroke="{{ in_array($field,['social_facebook','social_x','social_pixabay','social_wikipedia']) ? 'none' : 'currentColor' }}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">{!! $def['svg'] !!}</svg>
                        </span>
                        {{ $def['label'] }}
                        @if($field === 'social_email')
                        <span style="font-size:11px;color:#9ca3af;font-weight:400;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:120px;">{{ $author->$field }}</span>
                        @endif
                    </a>
                    @endif
                    @endforeach
        ...
        {{-- 작성 카테고리 --}}
        @if($categoriesWithArticles->count())
        <div class="j-card">
            ...
            <div class="j-card-body" style="padding:4px 14px;">
                <div class="j-cat-list">
                    ...
                    @php $cnt = \App\Models\Article::where('status','published')->where('user_id',$author->id)->where('category_id',$cat->id)->count(); @endphp
                    <div class="j-cat-item">
                        <a href="{{ route('news.index', ['category'=>$cat->slug]) }}">{{ $cat->name }}</a>
                        <span class="j-cat-count">{{ $cnt }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <main class="j-main">
        ...
        <div class="j-cat-articles">
            <div class="j-section-title">
                <span class="j-section-cat">{{ $cat->name }}</span>
                {{ $cat->name }} 최신 기사
                <a href="{{ route('news.index', ['category'=>$cat->slug]) }}" class="j-section-more">더보기 ›</a>
            </div>
            <div class="j-cat-grid">
                @foreach($cat->recentArticles as $a)
                <a href="{{ route('news.show', $a->slug) }}" class="j-cat-card">
                    @if($a->thumbnail)
                        <img src="{{ $a->thumbnail }}" alt="{{ $a->title }}" class="j-cat-card__img">
                    @else
                        <div class="j-cat-card__no-img">이미지 없음</div>
                    @endif
                    <div class="j-cat-card__body">
                        <div class="j-cat-card__title">{{ $a->title }}</div>
                        <div class="j-cat-card__date">{{ $a->published_at?->format('Y.m.d') ?? $a->created_at->format('Y.m.d') }}</div>
                    </div>
                </a>
                @endforeach
            </div>
        ...
        <div class="j-all-articles">
            <div class="j-section-title">
                전체 기사
                <span style="font-size:12px;font-weight:400;color:var(--j-text-sub);margin-left:4px;">{{ number_format($totalArticles) }}건</span>
            </div>

            <div class="j-article-grid">
                @forelse($articles as $a)
                <a href="{{ route('news.show', $a->slug) }}" class="j-article-row">
                    @if($a->thumbnail)
                        <img src="{{ $a->thumbnail }}" alt="{{ $a->title }}" class="j-article-row__img">
                    @else
                        <div class="j-article-row__no-img">No img</div>
                    @endif
                    <div class="j-article-row__body">
                        @if($a->category)
                            <span class="j-article-row__cat">{{ $a->category->name }}</span>
                        @endif
                        <div class="j-article-row__title">{{ $a->title }}</div>
                        <div class="j-article-row__meta">
                            {{ $a->published_at?->format('Y.m.d') ?? $a->created_at->format('Y.m.d') }}
                            &nbsp;·&nbsp; 조회 {{ number_format($a->view_count) }}
                        </div>
                    </div>
                </a>
                @empty
